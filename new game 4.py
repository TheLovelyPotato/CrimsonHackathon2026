...
# # Camera
# # ----------------------------
# capture = cv2.VideoCapture(1)  # change index based on cam num and position
...
# ----------------------------
# Player hitbox padding (in pixels)
RUN_PAD_X = 18
RUN_PAD_TOP = 10
RUN_PAD_BOTTOM = 10
...
# Paper obstacle hitbox padding
PAPER_PAD_X = 8
PAPER_PAD_Y = 8

# Bird obstacle hitbox padding (already shrunk, make a bit smaller)
HIGH_BIRD_PAD_X = 10
HIGH_BIRD_PAD_Y = 8
...
def spawn_obstacle(speed):
    ...
    if kind == OBS_PAPER:
        ...
        rect = pygame.Rect(WIDTH + 10, GROUND_Y - h, w, h)
        hit = shrink_hitbox(rect, PAPER_PAD_X, PAPER_PAD_Y)
        return {"kind": OBS_PAPER, "rect": rect, "hit": hit}, next_gap_px

    if kind == OBS_BIRD_HIGH:
        y_top = GROUND_Y - HIGH_BIRD_H - 58
        rect = pygame.Rect(WIDTH + 10, y_top, HIGH_BIRD_W, HIGH_BIRD_H)
        hit = shrink_hitbox(rect, HIGH_BIRD_PAD_X, HIGH_BIRD_PAD_Y)
        return {"kind": OBS_BIRD_HIGH, "rect": rect, "hit": hit}, next_gap_px
    ...
    rect = pygame.Rect(WIDTH + 10, low_bottom - LOW_BIRD_H, LOW_BIRD_W, LOW_BIRD_H)
    hit = shrink_hitbox(rect, LOW_BIRD_PAD_X, LOW_BIRD_PAD_Y)
    return {"kind": OBS_BIRD_LOW, "rect": rect, "hit": hit}, next_gap_px
...
while True:
    ...
    if not game_over:
        ...
        # Collision (use shrunk hitboxes when present)
        for obs in obstacles:
            if "hit" in obs:
                if dino_hitbox.colliderect(obs["hit"]):
                    game_over = True
